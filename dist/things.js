...
function propertiesRecord(fields) {
...
export async function getTodos(options = {}) {
    ...
    const source = options.project ? `to dos of project ${asString(options.project)}` :
...
export async function searchTodos(query, limit = 30) {
...
export async function createTodo(input) {
    ...
    const destination = input.project ? ` at beginning of project ${asString(input.project)}` :
...
export async function createArea(name, tags) {
    const props = propertiesRecord({
        name,
...
export async function createTag(name, parent) {
    const output = await runAppleScript(`
${appleScriptLibrary}
tell application "Things3"
...
export async function moveTodo(input) {
    const action = input.project ? `set project of targetToDo to project ${asString(input.project)}` :
...
export async function showItem(input) {
    const target = input.type === "todo" && input.id ? `my findTodoById(${asString(input.id)})` :
...
export async function quickEntry(input) {
    const props = propertiesRecord({
        name: input.title,
        notes: input.notes,
        "tag names": input.tags ? tagsText(input.tags) : undefined,
...
export async function searchInThings(query) {
...
export async function logCompletedNow() {
    return await runAppleScript(`
tell application "Things3"
  log completed now
